...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("AnyKey Coin Miner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("AnyKey Coin Miner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("AnyKey Coin Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("anykeycoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running AnyKey Coin Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
